...
/* 
 * rio_readcrlfb - Robustly read until CLRF (buffered)
 */
/* $begin rio_readcrlfb */
ssize_t rio_readcrlfb(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    int n, rc;
    char c, *bufp = (char *)usrbuf;
...
ssize_t Rio_readcrlfb(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    ...
    if ((rc = rio_readcrlfb(rp, usrbuf, maxlen)) < 0)
	unix_error("Rio_readcrlfb error");
    return rc;
} 
